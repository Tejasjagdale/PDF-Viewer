...
const RenderSection = (props: any) => {
  ...
  return (
    <>
      {parentId === -1 ? (
        <DragDropContext onDragEnd={handleDragDrop} onDragStart={(event: any) => handleClose(`${event.draggableId}`)}>
...
const CommonDropable = (props: any) => {
  const { data, expanded, children, parentId, handleClose, handleOpen, handleClick } = props.props;
                  ...
                  {(provided: any) => (
                    ...
                    >
                      <div
                        style={{
                          display: "flex",
                          paddingLeft: `${getChild(data, row.pdf_row_id)?.length ? 0 : 28}px`,
                        }}
                        onClick={handleClick}
                      >
                        <IconButton
